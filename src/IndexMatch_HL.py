...
# def save_filtered_geojson(geojson_data, output_folder, tile_id):
...
#     output_path = os.path.join(output_folder, f'Filtered_CensusTree_{tile_id}.geojson')
#     gdf.to_file(output_path, driver='GeoJSON')
#     print(f"Filtered CensusTree GeoJSON for tile {tile_id} saved to {output_path}")
...
for tile_folder in tqdm(tile_folders, desc="Processing progress: "):
    tile_dir = os.path.join(sample_dir, tile_folder)
    tile_start_time = time.time()
    print(f"\nProcessing tile: {tile_folder}")
    ...
    # save_filtered_geojson(filtered_geojson_data, os.path.join(tile_dir,f'GEOJSON_FilteredCensusTree_{tile_folder}'), tile_folder)
    # print(f"Total census trees: {len(filtered_geojson_data['features'])} ")
    ...
    print(f"\nTotal matched data: {len(matched_data)}")
    matched_data = post_process_matched_data(matched_data)
    # count_isNearest_true = sum(item['isNearest'] for item in matched_data if 'isNearest' in item)
    # print(f"Total matched censue trees: {count_isNearest_true}") 
    ...
    print(f"\nAverage time per tile: {avg_time_per_tile:.2f} seconds")
